...
#define VW_DEVICE_NAME		"/dev/aspeed-espi-vw"
...
static void SIOGetBMCVWValue(const int bitMask)
{
	int fd = -1;
	uint32_t readVirtualWireRegister;

	fd = open(VW_DEVICE_NAME, O_RDWR | O_CLOEXEC);
	if (fd < 0) {
		printf("Error open %s\n", VW_DEVICE_NAME);
		exit(1);
	}

	if (ioctl(fd, ASPEED_ESPI_VW_GET_GPIO_VAL, &readVirtualWireRegister) <
        0) {
		printf("Error in getting value %s\n", strerror(errno));
		close(fd);
		return;
	}

	int checkValue = readVirtualWireRegister & (1 << bitMask);
	printf("VW Register Bit %d is %s\n", bitMask, checkValue ? "set": "reset");
	close(fd);
}

static void SIOSetBMCVWValue(const int bitMask, bool set)
{
	int fd = -1;
	uint32_t writeVirtualWireRegister;
	uint32_t readVirtualWireRegister;
	...
	if (set ^ checkValue) {
		if (set){
			writeVirtualWireRegister = readVirtualWireRegister | (1 << bitMask);
		}

		else {
			writeVirtualWireRegister = readVirtualWireRegister & (~ (1 << bitMask));
		}
		if (ioctl(fd, ASPEED_ESPI_VW_PUT_GPIO_VAL, &writeVirtualWireRegister) < 
        0) {
			printf("Error in setting value %s\n", strerror(errno));
			close(fd);
			exit(-1);
		} 
	}

	printf("VW Register bit %d is %s\n", bitMask, set ? "set" : "cleared");
	close(fd);
}
...
static void usage(void)
{
	printf("Usage:\n"
	       ...
	       "\tlpc_cmds sio set_bmc_vw_value <gpio> <1|0>\n"
	       "\tlpc_cmds sio get_bmc_vw_value <gpio>\n"
	       "\n"
#if SUPPORT_KCS_ADDR_CMD
...
int main(int argc, char** argv)
{
	...
	if (strcmp(cmd, "sio") == 0) {
		...
		if (strcmp(argv[2], "get_acpi_state") == 0)
			...
			SIOSetBMCSMIEvent(0);
		else if (strcmp(argv[2], "get_bmc_vw_value") == 0) {
			if(argc < 4){
				usage();
			}
			char *endptr = NULL;
			errno = 0;
			long gpioIndex = strtol(argv[3], &endptr, 0);
			if(errno || *endptr || gpioIndex > 31 || gpioIndex < 0){
				printf("Invalid entry: GPIO Index must be between 0 and 31\n");
				usage();
			}
			SIOGetBMCVWValue(gpioIndex);
		}
		else if (strcmp(argv[2], "set_bmc_vw_value") == 0) {
			if(argc < 5){
				usage();
			}
			char *endptr = NULL;
			...
			endptr = NULL;
			errno = 0;
			long gpioVal = strtol(argv[4], &endptr, 0);
			if (errno || *endptr || gpioVal > 1 || gpioVal < 0){
				printf("Invalid entry: GPIO value must be 0 or 1\n");
				usage();
			}
			SIOSetBMCVWValue(gpioIndex,(bool)gpioVal);
		}
